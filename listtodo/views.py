...
def home(request):
    try:
        ...
        if authtoken !='':
            ...
            myProfile = usr.selfDetails()
        return JsonResponse([recentTweet,allProfile,myProfile], safe=False)
...
def signup(request):
    try:
        errorpage.printLog("errris comming do swagat")
...
class user:
    def __init__(self ,username,password=None,useremail=None,usern_mobile_no=None,userfullname = None,pic = None):
        try:
            ...
            return JsonResponse(json.dumps({"status": str(e)}), safe=False)
    ...
    def selfDetails(self):
        try:
            rs=Users.objects.filter(id = self.id)
            ans = []
            for row in rs:
                ans.append({
                    'id': row.id,
